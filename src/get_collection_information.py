...
COLLECTION_CALENDAR_URL = "https://www.recology.com/recology-king-county/shoreline/collection-calendar/"
ADDRESS = os.environ.get("address")

# Calendar Cell Dimensions (pixels)
...
def scrape_with_playwright():
    """
    Scrape the Recology King County Shoreline Collection Calendar using Playwright and BeautifulSoup.

    Returns:
        dict: A dictionary of weekly collection schedules grouped by start of the week (Monday).
    """
    if not ADDRESS:
        raise ValueError("Address not set in environment variables.")
    ...
    with sync_playwright() as p:
        ...
        page.goto(COLLECTION_CALENDAR_URL)
        ...
        dates = _extract_calendar_data(soup)
        events = _extract_event_data(soup)
        _match_events_to_dates(dates, events)
...
def _extract_calendar_data(soup):
    """
    Extracts all table cells with data-date attributes for the current month.

    Args:
        soup (BeautifulSoup): Parsed HTML of the page.

    Returns:
        dict: A dictionary with date strings as keys and an empty collection list as values.
    """
    dates = {}
...
def _extract_event_data(soup):
    """
    Extracts collection events from the calendar.

    Args:
        soup (BeautifulSoup): Parsed HTML of the page.

    Returns:
        list: A list of event dictionaries with type, left, and top attributes.
    """
    events = []
...
def _match_events_to_dates(dates, events):
    """
    Matches extracted events to their respective dates based on position.

    Args:
        dates (dict): Dictionary of dates with empty collections.
        events (list): List of event dictionaries with type, left, and top attributes.
    """
    for event in events:
        ...
        cells = list(dates.keys())  # Convert dict keys to a list for indexing
...
def _group_dates_into_weeks(dates):
    """
    Groups dates into weeks starting from Monday.

    Args:
        dates (dict): Dictionary of dates with collection information.

    Returns:
        dict: A dictionary of weeks with dates and their collections.
    """
    weeks = {}
    current_month = datetime.now().month
    ...
    # Filter out weeks with no dates from the current month
...
def _print_weeks(weeks):
    """
    Prints the weekly schedule to the console.

    Args:
        weeks (dict): A dictionary of weeks with dates and their collections.
    """
